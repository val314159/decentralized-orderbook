...
// added findLE and findGE - @val314159 (jmward@gmail.com)
...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function findLE(Tree storage self, uint key) internal view returns(uint) {
        require(key != EMPTY);
        if(exists(self, key))
	    return key;
	uint largest;
    ...
    function findGE(Tree storage self, uint key) internal view returns(uint) {
        require(key != EMPTY);
        if(exists(self, key))
	    return key;
	uint smallest;
